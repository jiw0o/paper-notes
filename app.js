...
const PRESENTATION_CATALOG_KEY = "paper-notes-presentations-v4";
...
$("#deck-frame").addEventListener("load", () => {
 try {
   ...
   if (frame.contentWindow.__presenterTools) return;

   const scriptText = `
     (function(){
       if (window.__presenterTools) return; window.__presenterTools = true;
       var ACCENT='#b3402e', OFFBG='rgba(20,20,22,0.72)';
       var laserOn=false, railVisible=true, t0=Date.now();
       try { if (localStorage.getItem('deck-stage.railVisible')==='0') railVisible=false; } catch(e){}
       var overlay, dot, bar, railBtn, laserBtn;
       ...
       function setLaser(on){
         ...
         laserBtn.style.background=on?ACCENT:OFFBG;
         laserBtn.title=on?'레이저 포인터 끄기 (Esc)':'레이저 포인터 켜기';
       }
       function setRail(on){
         railVisible=on;
         try{ window.postMessage({type:'__deck_rail_visible', on:on}, '*'); }catch(e){}
         railBtn.style.background=on?ACCENT:OFFBG;
         railBtn.title=on?'페이지 목록 접기':'페이지 목록 펼치기';
       }

       function build(){
         // Laser overlay: swallows clicks so the deck never advances; shows a dot.
         overlay=el('div','position:fixed;inset:0;z-index:2147483000;display:none;cursor:none;background:transparent;');
         overlay.id='__pt_laser_overlay'; overlay.setAttribute('data-omelette-chrome','');
         ...
         overlay.addEventListener('mousemove', moveDot);
         dot=el('div','position:fixed;left:-100px;top:-100px;width:22px;height:22px;margin:-11px 0 0 -11px;'
           +'border-radius:50%;pointer-events:none;z-index:2147483646;display:none;'
           +'background:radial-gradient(circle at 50% 50%, rgba(255,70,50,0.98) 0%, rgba(255,40,30,0.6) 38%, rgba(255,0,0,0) 72%);'
           +'box-shadow:0 0 16px 6px rgba(255,45,32,0.55);');
         dot.id='__pt_laser_dot';

         // Control bar (top-right, unobtrusive until hovered).
         bar=el('div','position:fixed;top:12px;right:12px;z-index:2147483647;display:flex;gap:8px;padding:6px;'
           +'border-radius:12px;background:rgba(20,20,22,0.55);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);'
           +'box-shadow:0 4px 18px rgba(0,0,0,0.32);opacity:0.4;transition:opacity .15s;');
         bar.id='__presenter_tools'; bar.setAttribute('data-omelette-chrome','');
         bar.addEventListener('mouseenter', function(){ bar.style.opacity='1'; });
         bar.addEventListener('mouseleave', function(){ bar.style.opacity='0.4'; });
         var bs='border:0;color:#fff;font:600 13px/1 -apple-system,BlinkMacSystemFont,sans-serif;padding:0 12px;'
           +'height:32px;border-radius:8px;cursor:pointer;display:inline-flex;align-items:center;';
         railBtn=el('button',bs,'▤ 목록'); laserBtn=el('button',bs,'● 레이저');
         railBtn.addEventListener('click', function(){ setRail(!railVisible); });
         laserBtn.addEventListener('click', function(){ setLaser(!laserOn); });
         bar.appendChild(railBtn); bar.appendChild(laserBtn);
         ...
         setLaser(false);
         railBtn.style.background=railVisible?ACCENT:OFFBG;
         railBtn.title=railVisible?'페이지 목록 접기':'페이지 목록 펼치기';
         window.addEventListener('keydown', function(e){ if(e.key==='Escape'&&laserOn) setLaser(false); });
         window.addEventListener('click', function(e){
           if (laserOn && !(e.target.closest && e.target.closest('#__presenter_tools'))) {
       ...
       function ensure(){
         if (document.getElementById('__presenter_tools')) return;
         if (!document.body) return;
         var rendered = !document.getElementById('__bundler_thumbnail') && document.body.children.length>0;
         if (!(rendered || (Date.now()-t0)>4000)) return;
         build();
       }
   ...
   console.error("Failed to inject presenter tools:", err);
 }
});
